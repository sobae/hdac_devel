...
int mc_GenerateConfFiles(const char *network_name)
{
    ...
    //const char * strDefaultConfFileName = "hdac";	// HDAC hdac.conf
    
    fileHan=mc_OpenFile(NULL,"hdac",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);	// HDAC
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(NULL,"hdac",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);	// HDAC
    }
    if(fileHan)
    {
    ...
    fileHan=mc_OpenFile(network_name,"hdac",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);	// HDAC
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(network_name,"hdac",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);	// HDAC
        if(fileHan)
        {
            fprintf(fileHan,"rpcuser=hdacrpc\n");	// HDAC
...
int mc_HdacParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
