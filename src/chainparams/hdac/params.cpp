...
#include "chainparams/hdac/paramlist.h"


int mc_OneHdacParam::IsRelevant(int version)
{
    int ret=1;
...
int64_t mc_HdacParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_HdacParams::GetDoubleParam(const char *param)
{
...
void* mc_HdacParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_HdacParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_HdacParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
int mc_HdacParams::ReParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_HdacParams::ReHashParams()
{    
    ...
    err=ReParam("chainparamshash",(const char*)hash,32);
    if(err)
    {
        return err;
    }
...
void mc_HdacParams::Init()
{
    ...
    m_Count=sizeof(HdacParamArray)/sizeof(mc_OneHdacParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((HdacParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(HdacParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(HdacParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OneHdacParam*)mc_New(sizeof(HdacParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,HdacParamArray,sizeof(HdacParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_HdacParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OneHdacParam *param;
    char *ptrData;
    int num_sets;
...
double mc_HdacParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


int mc_HdacParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_HdacParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    lpDefaultParams = new mc_HdacParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((HdacParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((HdacParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (HdacParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (HdacParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((HdacParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((HdacParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((HdacParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_HdacParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_HdacParams::Clone(const char* name, mc_HdacParams* source)
{
    int err;
...
int mc_HdacParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_HdacParams::Validate()
{
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"hdac") == 0)	// HDAC
            {
...
int mc_HdacParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== Hdac configuration file ====\n\n");	// HDAC
    fprintf(fileHan,"# Created by hdac-util \n");	// HDAC
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"hdacd %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());	// HDAC
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun hdac-util. \n");	// HDAC
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"hdacd %s\".\n",Name());	// HDAC
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"hdacd %s\".\n",Name());	// HDAC
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence hdac network configuration. \n");	// HDAC
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"hdac-util clone %s <new-network-name>\". \n",Name());	// HDAC
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running hdacd for this chain. \n");	// HDAC
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"hdac-util clone %s <new-network-name>\" to generate new network. \n",Name());	// HDAC
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by hdac-util.\n");	// HDAC
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by hdacd.\n");	// HDAC
...
int mc_HdacParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_HdacParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_HdacParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_HdacParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_HdacParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_HdacParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_HdacParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_HdacParams::ProtocolVersion()
{
