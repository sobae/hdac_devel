...
boost::thread* WebsocketThread = NULL;
#endif/* FEATURE_HPAY_WEBSOCKET */
...
#ifdef FEATURE_HPAY_WEBSOCKET
    strUsage += "  -wsport=<port>		 " + strprintf(_("Listen for Websocket connections on <port> (default: %u - disabled)"), 0) + "\n";
#endif/* FEATURE_HPAY_WEBSOCKET */
...
#ifdef FEATURE_HPAY_WEBSOCKET
    uint16_t wsport = GetArg("-wsport", 0);
    if(wsport)
    {
        websocket = new cWebsocket(wsport);
        
        if(websocket){
            WebsocketThread = new boost::thread(boost::bind(&WebsocketServer, boost::ref(websocket)));
        }
    }
#endif/* FEATURE_HPAY_WEBSOCKET */
