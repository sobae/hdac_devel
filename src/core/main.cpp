...
#ifdef FEATURE_HPAY_WEBSOCKET
#include "websocket/websocket.h"

extern cWebsocket* websocket;
#endif/* FEATURE_HPAY_WEBSOCKET */
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee,bool fAddToWallet)
{
    ...
    if(websocket)
    {
        websocket->wsBroadcastTx(tx, time(NULL));
        websocket->wsBroadcastMempool();
    }
#endif/* FEATURE_HPAY_WEBSOCKET */
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, int nHeight)	// HDAC
{
    ...
    if (!CheckProofOfWork(block.GetPoWHash(nHeight), block.nBits, block.nVersion, nHeight))	// HDAC
...
CAmount GetBlockValue(int nHeight, const CAmount& nFees) //HH
{
    ...
    #ifdef FEATURE_HPAY_CHANGED_BLOCK_REWARDS
    if(nHeight >= Params().GetStartHeightBlockRewardAdj())
    {
    	nSubsidy = nSubsidy / 2; 
    }
    #endif/* FEATURE_HPAY_CHANGED_BLOCK_REWARDS */
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    if(websocket)
    {
        websocket->wsBroadcastBlock(*pblock, pindexNew);
        websocket->wsBroadcastMempool();
    }
#endif/* FEATURE_HPAY_WEBSOCKET */
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    #ifdef FEATURE_HPAY_BLOCK_HASH_ALGORITHM
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(nHeight), block.nBits, block.nVersion, nHeight))	// HDAC
    ...
    // Check timestamp
    int64_t ts = Params().TargetSpacing();
    if(ts < 5)
    {
        ts = 5;
    }
    if (block.GetBlockTime() > GetAdjustedTime() + 2 * 6 * ts)
...
#ifdef FEATURE_HPAY_CHECK_BLOCK_MINER
...
#endif/* FEATURE_HPAY_CHECK_BLOCK_MINER */
