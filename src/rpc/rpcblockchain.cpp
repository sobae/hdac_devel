...
#ifdef FEATURE_HPAY_RPC_GETORPHANTX
struct COrphanTx {
    CTransaction tx;
    NodeId fromPeer;
};
...
#endif/* FEATURE_HPAY_RPC_GETORPHANTX */
...
#ifdef FEATURE_HPAY_RPC_GETORPHANTX
Value getorphantx(const Array& params, bool fHelp)
{
    ...
    if (fVerbose)
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(uint256, COrphanTx)& entry, mapOrphanTransactions)
        {
            const uint256& hash = entry.first;
            const CTransaction& tx = entry.second.tx;
            objTx.clear();
            TxToJSON(tx, uint256(0), objTx);
            objTx.push_back(Pair("hex", EncodeHexTx(tx)));
            txs.push_back(objTx);
    ...
    else
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(uint256, COrphanTx)& entry, mapOrphanTransactions)
        {
            ...
            a.push_back(tx.GetHash().ToString());
...
Array getmempoolStatus()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CTxMemPoolEntry)& entry, mempool.mapTx)
    {
        ...
        info.push_back(Pair("fee", ValueFromAmount(e.GetFee())));
        info.push_back(Pair("time", e.GetTime()));
        info.push_back(Pair("txid", hash.ToString()));
        a.push_back(info);
    }
    return a;
}
